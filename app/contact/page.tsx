        ...
        <div className="max-w-6xl mx-auto px-5 sm:px-8 lg:px-10">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-10 lg:gap-16">
            ...
            <motion.div initial={{ opacity: 0, x: -30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.8 }}>
              <span className="inline-block bg-green-100 text-green-800 text-xs font-bold tracking-widest uppercase px-4 py-1.5 rounded-full mb-4">Contact Info</span>
              <h2 className="text-2xl sm:text-3xl lg:text-4xl font-bold text-green-800 mb-4 sm:mb-5">Get In Touch</h2>
              ...
              <div className="space-y-5 sm:space-y-7">
                <div className="flex items-start gap-4">
                  <div className="bg-green-100 p-2.5 sm:p-3 rounded-xl flex-shrink-0"><MapPin className="text-green-700" size={20} /></div>
                  <div>
            ...
            <motion.div initial={{ opacity: 0, x: 30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.8 }}
              ...
              <h3 className="text-xl sm:text-2xl font-bold text-green-800 mb-1 sm:mb-2">Send Us a Message</h3>
              <p className="text-gray-400 text-xs sm:text-sm mb-6 sm:mb-7">Fill in the form and we'll get back to you within 24 hours.</p>

              <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-5">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    <label className="block text-xs font-semibold text-gray-500 uppercase tracking-wide mb-1.5">Full Name *</label>
                    <input type="text" name="name" placeholder="John Doe" required value={form.name} onChange={handleChange} className={inputClass} />
                  </div>
                  <div>
                    <label className="block text-xs font-semibold text-gray-500 uppercase tracking-wide mb-1.5">Email Address *</label>
                    <input type="email" name="email" placeholder="john@example.com" required value={form.email} onChange={handleChange} className={inputClass} />
                  </div>
                </div>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    <label className="block text-xs font-semibold text-gray-500 uppercase tracking-wide mb-1.5">Phone Number</label>
                    <input type="tel" name="phone" placeholder="+234 800 000 0000" value={form.phone} onChange={handleChange} className={inputClass} />
                  </div>
                  <div>
                    <label className="block text-xs font-semibold text-gray-500 uppercase tracking-wide mb-1.5">Service Interested In</label>
                ...
                <div>
                  <label className="block text-xs font-semibold text-gray-500 uppercase tracking-wide mb-1.5">Your Message *</label>
                  <textarea name="message" placeholder="Tell us about your inquiry, requirements, or how we can help…"
                    rows={5} required value={form.message} onChange={handleChange} className={`${inputClass} resize-none`} />
                </div>
        ...
        <div className="max-w-6xl mx-auto px-5 sm:px-8 lg:px-10">
          <div className="rounded-2xl sm:rounded-3xl overflow-hidden shadow-lg border border-gray-100 bg-green-50 h-48 sm:h-64 lg:h-80 flex items-center justify-center">
            <div className="text-center">
              ...
              <a href="https://maps.google.com/?q=Victoria+Garden+City+Lagos+Nigeria" target="_blank" rel="noopener noreferrer"
      ...
      <section className="py-12 sm:py-16 lg:py-20 bg-green-900 text-white">
        <div className="max-w-3xl mx-auto px-5 sm:px-8 lg:px-10 text-center">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.8 }}>
            <span className="inline-block bg-white/10 border border-white/20 text-green-200 text-xs font-bold tracking-widest uppercase px-4 py-1.5 rounded-full mb-4 sm:mb-5">Let's Connect</span>
            <h2 className="text-2xl sm:text-3xl lg:text-4xl font-bold mb-4 sm:mb-5 leading-tight">Ready to Partner With EMGO Farms?</h2>
